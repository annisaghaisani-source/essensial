...
test.describe('Validasi Limit 100 Kunjungan Pasien', () => {
    ...
    test('Add 100 Pasien Kunjungan Schedula', async ({ page }) => {
        ...
        for (let i = 1; i <= maxKunjungan; i++) {
            ...
            // Klik tombol Pendaftaran Baru
            await page.getByRole('button', { name: 'Pendaftaran Baru' }).first().click();
            await page.waitForTimeout(500);

            // Pilih 'Pendaftaran Baru' dari dropdown menu
            const pendaftaranMenu = page.getByRole('menuitem', { name: 'Pendaftaran Baru' });
            if (await pendaftaranMenu.count() > 0) {
                await pendaftaranMenu.first().click();
            } else {
                // Fallback click on text
                await page.getByText('Pendaftaran Baru', { exact: true }).nth(1).click();
            }
